...
void KBD_scan(void)
{
  ...
  if (but) {
    code = KBD_getBut(); 
    if (but == 1) {
	  if (code == BUT_RELEASED_CODE) { BEEP_beep(); RTOS_setTask(EVENT_KEY_SET, 0, 0); }
	  if (code == BUT_DOUBLE_CLICK_CODE) { BEEP_beep(); RTOS_setTask(EVENT_KEY_SET_DOUBLE, 0, 0); }
	  if (code == BUT_HELD_CODE) { BEEP_beep(); }
